...
public class MeasurementBasedApClientTPCTest {
	...
	void test1() throws Exception {
		...
		// Device A: no clients
		assertEquals(10, txPowerMap.get(deviceA).get(Constants.BAND_5G));

		// Device B: 1 client with RSSI -65
		assertEquals(14, txPowerMap.get(deviceB).get(Constants.BAND_5G));

		// Device C: 3 clients with min. RSSI -73
		assertEquals(26, txPowerMap.get(deviceC).get(Constants.BAND_5G));

		// Device D: 1 client with RSSI -80 => set to max txPower for MCS 7
		assertEquals(28, txPowerMap.get(deviceD).get(Constants.BAND_5G));

		// Device E: 1 client with RSSI -45 => set to min txPower
		assertEquals(TPC.MIN_TX_POWER, txPowerMap.get(deviceE).get(Constants.BAND_5G));
	}
}
